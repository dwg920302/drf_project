...
정해진 예산 안에서 구별로 다음 해에 배분할 기준을 마련하시오.
예산 금액을 입력하면, 구당 할당되는 CCTV 카운터를 자동으로 알려주는 AI 프로그램을 작성하시오.
'''
...
class Service(object):
    ...
    def save_cctv_pop(self):
        ...
        cctv = reader.from_csv(file)
        #   printer.dframe(cctv)

        file.fname = 'pop_in_seoul'
        print('-' * 50)
        pop = reader.from_xls(file, 2, 'B, D, G, J, N')
    ...
    def save_police_norm(self):
        ...
        police_pos = reader.from_csv(file)
        ...
        for i in self.arrest_rate_columns:
            ...
            # 데이터값의 기간 오류로 100을 넘으면 100으로 계산
        police.rename(columns={
            '살인 발생': '살인', '강도 발생': '강도',
            '강간 발생': '강간', '절도 발생': '절도', '폭력 발생': '폭력'
        }, inplace=True)
        ...
        police_norm['검거'] = np.sum(police_norm[self.arrest_rate_columns], axis=1)/len(self.arrest_rate_columns)
    ...
    def save_polium_map(self):
        ...
        file.fname = 'crime_in_seoul'
        crime = reader.from_csv(file)
        file.fname = 'kr_states'
        kr_states = reader.from_json(file)

        file.context = './saved_data/'
        file.fname = 'police_norm'
        police_norm = reader.from_csv(file)
        file.fname = 'police_pos'
        police_pos = reader.from_csv(file)
        # 시각화
        ...
        # Data는 Constant형이어야 하므로, Tuple형을 씀

        for i in police_pos.index:
            folium.CircleMarker(location=[police_pos['lat'][i], police_pos['lng'][i]],
